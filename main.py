    ...
    if request.args.get('wp'):
        if request.args.get('u'):
            ...
            for pwd in pw.read().splitlines():
                ...
                req = post(target, data=form, headers=usr_agent)
                w = req.text
                if kondisi in w:
                    return {
                        "status": 200,
...
@app.route("/api/hashdetect", methods=["GET", "POST"])
def hashdetect():
    if request.args.get("d"):
        ...
        if len(jerar)==0:
            ...
            for a in range(len(jerar)):
                return {
                    "status": 200,
                    "hash": f"{str(algorithms[jerar[a]])}"
                }
    else:
        return {
            "status": False,
            "message": "Masukkan parameter d"
        }
